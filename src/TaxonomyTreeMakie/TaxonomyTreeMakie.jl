"""
    PaleobiologyDB.Taxonomy.TaxonomyTreeMakie
...
# Standalone figure (returns Figure, Axis, TaxonomyTreePlot)
fig, ax, p = taxonomytreeplot(tree; showtips = true, color_by_rank = true)
...
fig3, ax3, p3 = taxonomytreeplot(taxon_subtree("Panthera"))
...
| Symbol | Description |
|---|---|
| `TaxonomyTreePlot` | Makie plot type (for dispatch / attribute access) |
| [`taxonomytreeplot`](@ref) | Create a standalone figure; returns `(Figure, Axis, TaxonomyTreePlot)` |
| `taxonomytreeplot!` | Add a dendrogram to an existing axis |
...
# level `import Makie` in TaxonomyTreeMakie.jl.
